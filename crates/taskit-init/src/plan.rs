...
pub struct InitPlan {
    /// Workspace crates to include in generated config.
    pub crates: Vec<CratePlan>,
    /// Dependency-propagation rules inferred from workspace graph.
    pub propagation: Vec<PropagationEntry>,
    /// Protocol surfaces to track for drift detection.
    pub surfaces: Vec<SurfacePlan>,
    /// Optional coverage defaults.
    pub coverage: Option<CoveragePlan>,
    /// CI steps for the generated pipeline.
    pub ci_steps: Vec<CiStepPlan>,
    /// Optional nextest expression skipped in offline mode.
    pub offline_skip: Option<String>,
    /// Optional flow branch configuration.
    pub flow: Option<FlowPlan>,
    /// Optional release settings.
    pub release: Option<ReleasePlan>,
    /// Whether to scaffold git hooks.
    pub git_hooks: bool,
    /// Whether to scaffold GitHub Actions CI.
    pub github_ci: bool,
    /// Whether to scaffold `deny.toml`.
    pub deny_toml: bool,
    /// Whether to scaffold `.ctx` project context files.
    pub ctx_scaffold: bool,
    /// Whether to scaffold mdBook docs.
    pub mdbook: bool,
...
pub struct CratePlan {
    /// Relative crate directory path.
    pub dir: String,
    /// Optional package name override.
    pub pkg: Option<String>,
}

/// Protocol surface entry for generated config.
#[derive(Debug, Clone)]
...
/// Coverage settings entry for generated config.
#[derive(Debug, Clone)]
...
/// CI step entry for generated config.
#[derive(Debug, Clone)]
pub struct CiStepPlan {
    /// Step display name.
    pub name: String,
    /// Step command key/string.
    pub cmd: String,
    /// Whether this step is a gate.
    pub gate: bool,
}

/// Branch names for generated flow configuration.
#[derive(Debug, Clone)]
pub struct FlowPlan {
    /// Main branch name.
    pub main: String,
...
/// Release section generated by init.
#[derive(Debug, Clone)]
pub struct ReleasePlan {
    /// Optional GitHub repo slug (`owner/name`).
    pub github_repo: Option<String>,
    /// Publish order for workspace crates.
