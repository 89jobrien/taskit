...
taskit changelog preview  # preview unreleased changelog entries
```

Subcommands are grouped by category: `dev` (build/cleanup/setup), `check` (fmt/lint/CI
...
`flow` (git branching), and `self` (taskit installation/testing/tool checks), plus top-level
`dashboard`, `init`, and `changelog`. Run `taskit --help` or `taskit <category> --help` to
see each group.
...
`release`, `flow`, and `self`, plus top-level `dashboard`, `init`, and `changelog`. Run
`taskit <category> --help` for the full flag list of any group.

### `dev` — build and workspace setup
...
| `check deps`                                                             | Check for unused dependencies (cargo-machete)       |
...
| `flow sync`    | Merge main into develop                                                                                                                          |
...
| `changelog full`                 | Regenerate CHANGELOG.md from complete git history    |
| `changelog latest`               | Prepend the latest tagged release                    |
| `changelog preview`              | Print unreleased changes without writing a file      |
